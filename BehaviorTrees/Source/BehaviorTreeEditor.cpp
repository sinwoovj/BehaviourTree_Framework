...
namespace BehaviorTreeEditor
{
  ...
  */
  ...
  void BuildTree(IBTNode * root, vector<Node> & nodes, vector<NodeLink> & links, int & id, ImVec2 pos = ImVec2(0, 0))
  {
	...
	if (node)
	{
	  ...
	  for (auto & child : node->m_children)
	  {
		...
		// make things more extreme the farther from the center they are
  ...
  static struct PossibleNodes
  {
	...
	std::vector<const char *> SelectorNodes;
  ...
  void GetPossibleNodes()
  {
	...
	possible_nodes.SelectorNodes.clear();
	possible_nodes.DecoratorNodes.clear();
	possible_nodes.LeafNodes.clear();
	// make copies in order to guarantee data will not change until this function is called
	...
	for (auto & node : possible_nodes.logicNodes_copy)
	{
	  ...
	  {
	  case NT_Selector:
		possible_nodes.SelectorNodes.push_back(node.first.c_str());
		break;
	  case NT_Decorator:
		...
		if (currentItem < 4 && currentItem >= 0)
		{
		  ...
		  if (type == NT_Selector)
		  {
			...
			if (ImGui::Combo("##Selectors", &selected,
			  const_cast<const char **>(possible_nodes.SelectorNodes.data()),
			  static_cast<int>(possible_nodes.SelectorNodes.size())))
			{
			  std::strcpy(buf, possible_nodes.SelectorNodes[selected]);
			}
		  }
		  ...
		  if (ImGui::Button("Add"))
		  {
			if (std::strcmp(buf, "") != 0)
			{
			  ...
			  if (parent)
			  {
				...
				{
				  new_node = createLogicNode(std::string(possible_nodes.SelectorNodes[selected]), std::string(buf));
				}
				if (new_node)
				{
