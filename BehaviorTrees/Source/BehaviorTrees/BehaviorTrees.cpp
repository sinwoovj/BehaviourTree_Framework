...
bool BehaviorTrees::Unregister(const std::string &agentname)
{
	...
	GameObject *object = m_agentdata[agent_index]->GetGameObject();
...
{
	std::vector<std::string> delete_agents;

	for (unsigned i = 0; i < m_agentdata.size(); ++i)
	{
		m_agentdata[i]->Execute(dt);

		// if the agent is marked as deleted, add to the string list for deletion later
		if (m_agentdata[i]->IsMarkForDeletion())
		{
			delete_agents.push_back(m_agentdata[i]->GetName());
		}
	}

	if (delete_agents.size())
	{
		for (std::vector<std::string>::iterator it = delete_agents.begin(); it != delete_agents.end(); ++it)
		{
			Unregister(*it);
		}
	}
